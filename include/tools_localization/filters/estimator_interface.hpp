...
inline string get_ground_truth(const string& path_to_measurements) {
    // the files generated by the simulator have a matching ground truth file that's associated
    // by a shared timestamp
    ...
    if (found == string::npos) {
        throw std::runtime_error("Expected 'test_' in filename but did not find it.");
    }
    ...
    throw std::runtime_error("No matching ground truth file found for: " + path_to_measurements);
}
